...
// Ensure LooseTopologicalNode is the holder type
using LooseTopologicalNode = torch::nn::ModuleHolder<LooseTopologicalNodeImpl>;

PYBIND11_MODULE(LooseTopologicalNode, m) {
    m.doc() = "LooseTopologicalNode PyTorch C++ extension module";

    // Bind the implementation class (inherits from torch::nn::Module)
    ...
    // Bind the holder class and specify torch::nn::Module as Python base
    py::class_<LooseTopologicalNode, torch::nn::ModuleHolder<LooseTopologicalNodeImpl>, torch::nn::Module, std::shared_ptr<LooseTopologicalNode>>(m, "LooseTopologicalNode")
        .def(py::init<int, int>(), "Simplified constructor (input_dim, output_dim)")
        .def(py::init<float, int, int, int, int>(), "Full constructor (leak_factor, input_dim, output_dim, num_hidden_layers, hidden_layer_size)")
        .def("forward", &LooseTopologicalNode::forward, "Forward method (calls Impl internally)");
}
